...
class DataSeeder:
    def __init__(self):
        ...
        self.mongo_client = MongoClient("mongodb://mongodb:mongodb@localhost:27017/?authSource=admin")
        self.mongo_db = self.mongo_client["mongo_store"]

        self.mysql_engine = create_engine("mysql+mysqlconnector://mysql:mysql@localhost:3306/mysql_store")

        self.postgres_engine = create_engine("postgresql+psycopg2://postgres:postgres@localhost:5432/postgres")
    ...
    def to_csv(self, df: pd.DataFrame, filename: str) -> Dict:
        ...
        try:
            ...
            report = {"destination": "CSV", "status": "failed", "file": filename, "error": str(e)}
    ...
    def to_excel(self, df: pd.DataFrame, filename: str) -> Dict:
        ...
        try:
            ...
            report = {"destination": "Excel", "status": "failed", "file": filename, "error": str(e)}
            raise Exception(report)
    ...
    def to_mongo(self, df: pd.DataFrame, filename: str) -> Dict:
        ...
        try:
            ...
            return {"destination": "MongoDB", "status": "success", "file": filename, "records": len(data_dict)}
        except Exception as e:
            report = {"destination": "MongoDB", "status": "failed", "file": filename, "error": str(e)}
            raise Exception(report)
    ...
    def to_mysql(self, df: pd.DataFrame, filename: str) -> Dict:
        ...
        try:
            ...
            df.to_sql(name=table_name, con=self.mysql_engine, if_exists='replace', index=False)

            return {"destination": "MySQL", "status": "success", "file": filename}
        except Exception as e:
            report = {"destination": "MySQL", "status": "failed", "file": filename, "error": str(e)}
            raise Exception(report)
    ...
    def to_postgres(self, df: pd.DataFrame, filename: str) -> Dict:
        ...
        try:
            table_name = filename.split(".")[0]
            df.to_sql(name=table_name, con=self.postgres_engine, if_exists='replace', index=False)

            return {"destination": "PostgreSQL", "status": "success", "file": filename}
        except Exception as e:
            report = {"destination": "PostgreSQL", "status": "failed", "file": filename, "error": str(e)}
            raise Exception(report)


    def to_sqlite(self, df: pd.DataFrame, filename: str) -> Dict:
        try:
            table_name = filename.split(".")[0]
            df.to_sql(name=table_name, con=self.sqlite_engine, if_exists='replace', index=False)
        except Exception as e:
            report = {"destination": "Sqlite", "status": "failed", "file": filename, "error": str(e)}
            raise Exception(report)

    def seed_file_parallel(self, filename: str, destinations: List[str] = None) -> List[Dict]:
        """
    ...
    def seed_all_files(self, destinations: List[str] = None, max_workers: int = 3):
        """
